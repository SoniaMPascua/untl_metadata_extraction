...
metadata_fields = ["title", "creator", "contributor", "publisher", "date", \
            "language", "description", "subject", "primarySource", "coverage", \
             "source", "citation", "relation", "collection", "institution", \
             "rights", "resourceType", "format", "identifier", "degree", "note", \
            "meta"]
...
for filename in sys.stdin:
    ...
    if rd["metadata_creation_date"] != "None":
        creation_time = datetime.strptime(rd["metadata_creation_date"], "%Y-%m-%d, %H:%M:%S")

    if rd["metadata_edit_date"] != "None":
        #2014-03-12, 11:42:25
        edit_time = datetime.strptime(rd["metadata_edit_date"], "%Y-%m-%d, %H:%M:%S")
    ...
    #w = csv.DictWriter(sys.stdout, rd.keys())
    #w.writeheader()
    #w.writerow(rd)
    if header == True:
